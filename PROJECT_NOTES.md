...
- Runs `pythonw.exe desk_lamp.py --rainbow -t 63.7 --log` from the project folder, so there is no console window. Change the lap time or any rainbow knob with e.g. `-Arguments '--rainbow -t 90 --dwell red=6'` and re-run the script.
