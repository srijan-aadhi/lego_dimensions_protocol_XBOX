...
- **Rainbow taste knobs (added 2026-09-23):** even perceived pacing still *felt* slow through green-blue, because the eye has one name ("blue") for a stretch as wide as red+orange+yellow. So three optional adjustments, all smooth raised-cosine windows over HSV hue:
  ...
  - `--dwell COLOUR=X` (repeatable): linger X times longer around a hue, half-width 30°. COLOUR is a name (red, orange, yellow, lime, green, teal, cyan, azure, blue, violet, magenta, rose; 30° apart starting at red = 0°) or degrees. Defaults: `red=5`, `yellow=1.5`, `teal=1.8`; a given `--dwell` overrides that colour's default, `red=1` = even. Perceived hue barely changes across red, so even pacing gave it only ~2.5 s of a 60 s lap versus ~11 s for orange. `teal` is the green->cyan transition, which looked too brief. (`--hold` is a different option: seconds to stay on each colour in multi-colour mode.)
  - **Tuned by eye on 2026-09-23** and baked in as the defaults, so the start-up task only passes `--rainbow -t 60`. The table is renormalised, so raising one dwell shortens everything else a little; to add time to one colour *without* touching the others, lengthen the lap by the same proportion instead. That is why the start-up lap is **62.3 s**: it is the 60 s lap plus exactly the time the teal dwell adds. Resulting seconds per lap (30° bands): red 7.1, orange 10.0, yellow 7.4, lime 2.7, green 0.5, teal 4.6, cyan 7.7, azure 3.7, blue 1.2, violet 5.6, magenta 4.3, rose 7.5.
...
- Runs `pythonw.exe desk_lamp.py --rainbow -t 62.3 --log` from the project folder, so there is no console window. Change the lap time or any rainbow knob with e.g. `-Arguments '--rainbow -t 90 --dwell red=6'` and re-run the script.
