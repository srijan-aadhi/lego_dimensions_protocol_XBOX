...
- **Rainbow taste knobs (added 2026-09-23):** even perceived pacing still *felt* slow through green-blue, because the eye has one name ("blue") for a stretch as wide as red+orange+yellow. So three optional adjustments, all smooth raised-cosine windows over HSV hue:
  ...
  - `--red-hold X` (default 5) and `--yellow-hold X` (default 1.5): linger X times longer around pure red (centre 0°) / pure yellow (centre 60°), half-width 30°. Perceived hue barely changes across red, so even pacing gave it only ~2.5 s of a 60 s lap versus ~11 s for orange. Pure green and blue are similarly brief (~1 s) but have not bothered anyone.
  - **Tuned by eye on 2026-09-23** and baked in as the defaults, so the start-up task only passes `--rainbow -t 60`. Resulting seconds per 60 s lap (30° bands): red 7, orange 10, yellow 7.5, lime 3, green 0.4, spring 3, cyan 7.5, azure 4, blue 1, violet 5, magenta 4, rose 7; warm 34 s / cool 26 s. Each knob is a smooth raised-cosine window, and the table is renormalised, so raising one hold shortens everything else a little.
...
- Runs `pythonw.exe desk_lamp.py --rainbow -t 60 --log` from the project folder, so there is no console window. Change the lap time or any rainbow knob with e.g. `-Arguments '--rainbow -t 90 --red-hold 6'` and re-run the script.
