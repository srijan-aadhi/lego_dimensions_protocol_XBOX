...
# Switch all pads off (Found by fuzzing)
ALL_PADS_OFF = [0x55, 0x06, 0xc0, 0x02, 0x00, 0x00, 0x00, 0x00, 29, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]
...
def send_commands(name, commands, delay=2):
    """Send a list of raw commands to the portal, pausing between each"""
    print(name)
...
def cycle_fade():
    ...
    send_commands("cycle_fade", commands)
...
def cycle_immediate_all():
    ...
    commands = [
        ...
        ALL_PADS_OFF,  # Switch all pads off (Found by fuzzing)
    ]
    send_commands("cycle_immediate_all", commands)


def cycle_immediate_individual():
    """Cycle through immediate switch of individual pad colours"""
    commands = [
        # Center
    ...
    send_commands("cycle_immediate_individual", commands)
...
def main():
    ...
    try:
        ...
        # Leave the pads dark when the user stops the demo with Ctrl+C
        dev.write(1, ALL_PADS_OFF)
        print("\nStopped")
